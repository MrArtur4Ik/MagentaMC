...
import packets, utils, server, socket, threading, random
...
def command_handler(sender: CommandSender, command: str, args: list):
	if isinstance(sender, Player):
		print(sender.nickname, "executed command: /" + command, " ".join(args))
	if command == "list":
		players = []
		for player in get_players():
...
def packet_handler(connection: socket.socket, client_address: tuple, p: packets.PacketSerializer):
	if isinstance(p, packets.PlayerIdentification):
		print(client_address, "authorized as", p.username)
...
class ServerHandler(StreamRequestHandler):
	...
	def handle(self):
		...
		print(self.client_address, "connected. Auth...")
		try:
			while True:
				try:
